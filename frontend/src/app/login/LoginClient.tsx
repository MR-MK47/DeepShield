...
export default function LoginClient() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    alert('Authentication stub: Use the scan console directly.')
      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-6 space-y-5">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Enter your operator credentials</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            ...
            <div className="flex items-center justify-between pt-1">
              <label className="flex items-center gap-2 text-slate-400 cursor-pointer">
                <input type="checkbox" className="w-3.5 h-3.5 rounded border-slate-800 bg-slate-900 text-blue-500 focus:ring-0" />
                Remember session
              </label>
          ...
          >
            Open Scan Console Without Login
          </Link>
        </div>
      </div>
    </div>
